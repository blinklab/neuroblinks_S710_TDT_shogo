...
% Last Modified by GUIDE v2.5 04-Dec-2015 16:23:18
...
sortcodes=get(handles.listbox_snip,'String');
ID_snip = sortcodes(get(handles.listbox_snip,'Value'),:);
[ts,waves,sortcodes]=TDTgetSpikeData(TTX,ID_snip);
set(handles.listbox_snips_spk,'String',unique(sortcodes));
...
if strcmpi(selectedunit,'N'),
    sortcodes=get(handles.listbox_snips_spk,'String');
    if strcmpi(sortcodes(get(handles.listbox_snips_spk,'Value')),'0'),
        return,
    end
end
...
% --- Executes on selection change in listbox_snips_spk.
function listbox_snips_spk_Callback(hObject, eventdata, handles)
% hObject    handle to listbox_snips_spk (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: contents = cellstr(get(hObject,'String')) returns listbox_snips_spk contents as cell array
%        contents{get(hObject,'Value')} returns selected item from listbox_snips_spk
ghandles=getappdata(0,'ghandles');
ghandles.flttrgnum=0;
setappdata(0,'ghandles',ghandles);
...
% --- Executes during object creation, after setting all properties.
function listbox_snips_spk_CreateFcn(hObject, eventdata, handles)
% hObject    handle to listbox_snips_spk (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
% [spiketimes,spikeshapes,sortcodes]=TDTgetSpikeData(TTX,ID_snip,sttime,endtime,'FILTERED');
...
each_dur=50;
rep=floor((endtime-sttime)/each_dur);
sortcodes=get(handles.listbox_snip,'String');
ID_snip = sortcodes(get(handles.listbox_snip,'Value'),:);
